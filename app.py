...
# 常數：圖片 URL（直接連結）
# ─────────────────────────────────────────────
_IMG_BASE = "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/"
_PREVIEW_BASE = _IMG_BASE + "preview/"

# 主要常數（超過 1MB 的 PNG 一律改用壓縮後的 JPG）
AKA_IMAGE_URL = _IMG_BASE + "aka.jpg"
STORE_IMAGE_URL = _IMG_BASE + "IMG-6192.jpg"
TEAM_PHOTO_URL = _IMG_BASE + "team-photo.jpg"
AKA_BADMINTON_URL = _IMG_BASE + "aka_badminton.jpg"


def _get_preview_url(original_url: str) -> str:
    """
    從 original URL 自動推導 preview URL。
    規則：取檔名（不含副檔名）+ _preview.jpg，放在 preview/ 子資料夾。
    例如：.../images/aka.jpg → .../images/preview/aka_preview.jpg
          .../images/master-dake.jpg → .../images/preview/master-dake_preview.jpg
    """
    if _IMG_BASE not in original_url:
        return original_url  # 非本 repo 圖片，原樣回傳
    filename = original_url.split("/")[-1]
    name_no_ext = filename.rsplit(".", 1)[0]
    return _PREVIEW_BASE + name_no_ext + "_preview.jpg"
...
INTERCEPT_MAP = {
    "[選單-優惠&活動]": {
        "text": "阿卡幫你找了好康... 🥰 可是現在活動好多喜... 你想看專屬的『VIP優惠』👑、超劃算的『換購活動』🎁，還是最新的『伸懶腰百日慶活動』🎉呢？跟阿卡說喜...🦥",
        "image_url": _IMG_BASE + "aka.jpg"
    },
    "[選單-優惠活動]": {
        "text": "阿卡幫你找了好康... 🥰 可是現在活動好多喜... 你想看專屬的『VIP優惠』👑、超劃算的『換購活動』🎁，還是最新的『伸懶腰百日慶活動』🎉呢？跟阿卡說喜...🦥",
        "image_url": _IMG_BASE + "aka.jpg"
    ...
    "[選單-店內資訊]": {
        ...
        "image_url": _IMG_BASE + "aka-system-building.jpg"
    },
    "[選單-交通&位置]": {
        "text": "我們在東光路852巷20號1樓... 🦥\n跟著地圖走就不會迷路囉 👉 https://maps.app.goo.gl/f7Br1zswqzTuWxr36\n慢慢走過來，我們在這裡等你... 🌿",
        "image_url": _IMG_BASE + "IMG-6192.jpg"
    },
    "[選單-服務/團隊]": {
        "text": "阿卡伸個懶腰...🥱 我們的服務分好多種喜... 你想要看完整的「總價目表」📋、要阿卡直接幫你「推薦套餐」🌿，還是想認識我們的「伸懶腰專業團隊」👥呢？跟阿卡說喜...🦥",
        "image_url": _IMG_BASE + "aka-service.jpg"
    },
}
                ...
                PushMessageRequest(
                    ...
                    messages=[
                        ImageMessage(
                            ...
                            preview_image_url=_get_preview_url(AKA_BADMINTON_URL)
                        ),
    ...
    if stripped in INTERCEPT_MAP:
        ...
        messages = [
            ...
            ImageMessage(
                ...
                preview_image_url=_get_preview_url(img_url)
            )
        ]
...
def check_package_flow(user_id: str, text: str):
    ...
    if any(kw in stripped for kw in _PACKAGE_TRIGGER_KEYWORDS):
        recommend_url = _IMG_BASE + "aka-recommend.jpg"
        USER_SESSION[user_id] = "awaiting_package_interest"
        return [
            TextMessage(
                text="好喜...🥱 每個人的狀況不一樣... 你是第一次來嗎？"
                     "或者想試試『深層肌筋膜油推』？還是特別的『頭部整復SPA』...?🦥"
            ),
            ImageMessage(
                original_content_url=recommend_url,
                preview_image_url=_get_preview_url(recommend_url)
            ),
...
- aka.jpg                   → 阿卡主圖、歡迎、優惠活動入口
- aka-service.jpg           → 服務選單入口圖
- aka-recommend.jpg         → 推薦套餐入口圖
- aka-system-building.jpg   → 店內資訊/系統建置中
- vip-promo.jpg             → VIP 優惠活動
- exchange-1.jpg            → 換購活動圖1（搭配 exchange-2.png 一起發）
- exchange-2.png            → 換購活動圖2
- centenary-1.jpg           → 百日慶活動圖1（搭配2、3一起發）
- centenary-2.jpg           → 百日慶活動圖2
- centenary-3.jpg           → 百日慶活動圖3
- IMG-6192.jpg              → 交通位置地圖
- IMG-6739.jpg              → 總價目表圖1（搭配 IMG-6738.jpg 一起發）
- IMG-6738.jpg              → 總價目表圖2
- package-pricelist.jpg     → 完整版三族群套餐價目表（上班小資族/運動修復/重度勞動者）
- IMG-6995.jpg              → 第一次來推薦套餐
...
- master-ayu.jpg            → 師傅阿瘀介紹圖
- master-dake.jpg           → 師傅大可介紹圖
- master-aya.jpg            → 師傅阿YA介紹圖
- master-yunyun.jpg         → 師傅芸芸介紹圖
- master-ajun.jpg           → 師傅阿駿介紹圖

完整 URL 格式範例：https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/aka.jpg

【圖文選單與互動對話邏輯】

🟢 情境一：優惠活動引導
1. 客人輸入「[選單-優惠&活動]」或詢問「優惠」、「活動」時：
   {"text": "阿卡幫你找了好康... 🥰 可是現在活動好多喞... 你想看專屬的『VIP優惠』👑、超劃算的『換購活動』🎁，還是最新的『伸懶腰百日慶活動』🎉呢？跟阿卡說喞...🦥", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/aka.jpg", "action": "none", "notify_admin": false}
...
3. 客人回覆「換購」或「換購活動」：
   {"text": "這個換購活動超讚的...🥱 來放鬆還可以順便帶超值好禮回家... 你看看有沒有喜歡的...🎁", "image_urls": ["https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/exchange-1.jpg","https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/exchange-2.png"], "action": "none", "notify_admin": false}

4. 客人回覆「百日慶」、「伸懶腰百日慶」或「最新活動」：
   {"text": "耶...我們滿一百天了...🎉 這是最新的百日慶特別活動喞... 阿卡準備了滿滿的驚喜給你...來看看吧...🦥✨", "image_urls": ["https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/centenary-1.jpg","https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/centenary-2.jpg","https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/centenary-3.jpg"], "action": "none", "notify_admin": false}

5. 客人輸入「[選單-店內資訊]」或問店內資訊：
   {"text": "有任何想知道的資訊可以先看我們的選單，或是要稍待片刻，阿卡去請師傅來 😃", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/aka-system-building.jpg", "action": "none", "notify_admin": false}
...
1. 客人輸入「[選單-服務/團隊]」或問服務：
   {"text": "阿卡伸個懶腰...🥱 我們的服務分好多種喞... 你想要看完整的「總價目表」📋、要阿卡直接幫你「推薦套餐」🌿，還是想認識我們的「伸懶腰專業團隊」👥呢？跟阿卡說喞...🦥", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/aka-service.jpg", "action": "none", "notify_admin": false}
...
4. 客人回覆「第一次來」：
   {"text": "歡迎第一次來...🦥 阿卡幫你推薦最適合新朋友的入門體驗...慢慢感受一下...🌿", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/IMG-6995.jpg", "action": "none", "notify_admin": false}
    ...
    for url in all_image_urls:
        ...
        if any(lower_url.endswith(ext) for ext in [".jpg", ".jpeg", ".png", ".gif", ".webp"]):
            ...
            messages.append(ImageMessage(
                ...
                preview_image_url=_get_preview_url(url)
            ))
        else:
            ...
            ReplyMessageRequest(
                ...
                messages=[
                    ...
                    ImageMessage(
                        ...
                        preview_image_url=_get_preview_url(AKA_IMAGE_URL)
                    )
                ]
            )
        )
