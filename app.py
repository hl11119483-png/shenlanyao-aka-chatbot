...
# 主要常數（透明背景插圖用 PNG，實景照片用 JPG）
AKA_IMAGE_URL = _IMG_BASE + "aka.png"
...
# 有透明背景的 PNG 檔名清單（preview 也要用 .png 保留透明）
_TRANSPARENT_PNG_NAMES = {
    "aka", "aka-recommend", "aka-service", "aka-system-building", "exchange-1"
}
...
def _get_preview_url(original_url: str) -> str:
    ...
    透明背景 PNG → preview 用 .png，其他 → preview 用 .jpg。
    例如：.../images/aka.png → .../images/preview/aka_preview.png
    ...
    ext = ".png" if name_no_ext in _TRANSPARENT_PNG_NAMES else ".jpg"
    return _PREVIEW_BASE + name_no_ext + "_preview" + ext
...
INTERCEPT_MAP = {
    ...
    "[選單-店內資訊]": {
        ...
        "image_url": _IMG_BASE + "aka-system-building.png"
    },
    "[選單-交通&位置]": {
    ...
    "[選單-服務/團隊]": {
        ...
        "image_url": _IMG_BASE + "aka-service.png"
    },
    # 團隊大合照直接觸發關鍵字
    "團隊": {
        "text": "這是我們超專業的團隊喔...🌿 你想認識哪一位？還是跟阿卡說你哪裡痠痛、怕不怕痛，阿卡幫你推薦...🥱",
        "image_url": _IMG_BASE + "team-photo.jpg"
    },
    "伸懶腰專業團隊": {
        "text": "這是我們超專業的團隊喔...🌿 你想認識哪一位？還是跟阿卡說你哪裡痠痛、怕不怕痛，阿卡幫你推薦...🥱",
        "image_url": _IMG_BASE + "team-photo.jpg"
...
# 整復推拿關鍵字匹配集合（含部分匹配）
_ZHENGFU_KEYWORDS = {"整復推拿", "整復", "推拿", "傳統整復", "傳統推拿", "整復師傅", "推拿師傅"}
_ZHENGFU_TEXT = (
    "伸懶腰的傳統整復推拿超有效的...🥱 手法湫和精準，專門針對筋骨、肌肉、經絡進行調理...🌿\n\n"
    "我們的整復推拿服務包含：\n"
    "👉 傳統整復推拿（筋骨調整、經絡疏通）\n"
    "👉 深層肌筋膜油推（放鬆深層緊纃）\n"
    "👉 頭部整復SPA（腦部放鬆、改善睡眠）\n\n"
    "你哪裡最不舒服呢...？🦥 跟阿卡說說，阿卡幫你找最適合的師傅和方案...🌿"
)
...
def check_intercept(text: str):
    ...
    # 整復推拿關鍵字（含部分匹配）
    if any(kw in stripped for kw in _ZHENGFU_KEYWORDS):
        return [TextMessage(text=_ZHENGFU_TEXT)]
...
def check_package_flow(user_id: str, text: str):
    ...
    if any(kw in stripped for kw in _PACKAGE_TRIGGER_KEYWORDS):
        recommend_url = _IMG_BASE + "aka-recommend.png"
...
- aka.png                   → 阿卡主圖、歡迎、優惠活動入口（透明背景）
- aka-service.png           → 服務選單入口圖（透明背景）
- aka-recommend.png         → 推薦套餐入口圖（透明背景）
- aka-system-building.png   → 店內資訊/系統建置中（透明背景）
- vip-promo.jpg             → VIP 優惠活動
- exchange-1.png            → 換購活動圖1（透明背景，搭配 exchange-2.png 一起發）
...
1. 客人輸入「[選單-優惠&活動]」或詢問「優惠」、「活動」時：
   {"text": "阿卡幫你找了好康... 🥰 可是現在活動好多喞... 你想看專屬的『VIP優惠』👑、超劃算的『換購活動』🎁，還是最新的『伸懶腰百日慶活動』🎉呢？跟阿卡說喞...🦥", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/aka.png", "action": "none", "notify_admin": false}
...
1. 客人輸入「[選單-服務/團隊]」或問服務：
   {"text": "阿卡伸個懶腰...🥱 我們的服務分好多種喞... 你想要看完整的「總價目表」📋、要阿卡直接幫你「推薦套餐」🌿，還是想認識我們的「伸懶腰專業團隊」👥呢？跟阿卡說喞...🦥", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/aka-service.png", "action": "none", "notify_admin": false}
...
7. 客人回覆「上班小資族」：
   {"text": "阿卡上班族最需要這個了...🥱 肩頸、腰背的疲憊通通幫你舒緩...💆‍♀️", "image_url": "https://raw.githubusercontent.com/hl11119483-png/shenlanyao-aka-chatbot/main/assets/images/IMG-7137.jpg", "action": "none", "notify_admin": false}
